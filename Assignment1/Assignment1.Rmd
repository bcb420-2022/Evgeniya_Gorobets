...
```{r}
# A helper that will return only the columns in gseData that have count data, 
# removing any columns with gene names and symbols
getGSECounts <- function() {
  gseCounts <- gseData[, 3:8]
  rownames(gseCounts) <- gseData$Ensembl
  return(gseCounts)
}
