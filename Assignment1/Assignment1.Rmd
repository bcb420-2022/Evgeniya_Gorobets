...
dataset. However, I can use the HGNC symbols from the original dataset to 
gain insight to my unmapped genes.

```{r}
# Replace original HGNC mappings with the ones I retrieved
gseData <- merge(
  geneIdMappings, gseData, by.x='ensembl_gene_id', by.y='Ensembl', all.y=T)
colnames(gseData)[colnames(gseData) == "ensembl_gene_id"] <- "Ensembl"

# Identify unmapped genes
unmappedRows <- is.na(gseData$hgnc_symbol)
unmappedGenes <- gseData[unmappedRows, c("Ensembl", "HGNC", "hgnc_symbol")]

# What percentage of genes are unmapped?
100 * nrow(unmappedGenes) / nrow(gseData)
```
806 of 14,241 genes are unmapped. This corresponds to 5.66% of all the genes in 
my dataset. First, I check whether the unmapped genes are ribosomal proteins, 
which do not affect differential expression analysis.

```{r}
# How many are ribosomal proteins?
length(grep("^RP", unmappedGenes$HGNC))
```

Only 4 of the unmapped genes are ribosomal proteins, so there are 802 other 
genes that are unmapped. To try and investigate, I will look up the first few 
unmapped genes in the Ensembl database.
...
```{r}
# Remove old HGNC symbols
gseData <- gseData[, names(gseData) != "HGNC"]

# Save data
# cnm = cleaned, normalized, mapped
dataPath <- paste0(DATA_DIR, "/cnmData.rds")
saveRDS(gseData, dataPath)
```
<br/>
